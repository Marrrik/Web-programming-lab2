...
laba6 = Blueprint('laba6', __name__)

@laba6.route('/lab6/check')
def main():
    my_users = users.query.all()
    print(my_users)
    return 'console'

@laba6.route('/lab6/checkarticles')
def checkarticles():
    my_articles = articles.query.all()
    print(my_articles)
    return 'console'

@laba6.route('/lab6/registr', methods=["GET", "POST"])
def registrPage():
    errors = []
...
@laba6.route('/lab6/login', methods = ['GET', 'POST'])
def login():
    errors = []
    if request.method == 'GET':
...
@laba6.route('/lab6/logout', methods=['GET', 'POST'])
@login_required
def logout():
    logout_user()
    return redirect('/lab6/glav')
...
@laba6.route("/lab6/articles/<int:article_id>")
@login_required
...
@laba6.route("/lab6/newtitle", methods=["GET", "POST"])
@login_required
def createArticle():
    if request.method == "GET":
